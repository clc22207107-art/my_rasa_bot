...
def entity_per_type(expected: list, got: list) -> dict:
    """
    Trả về dict {entity_type: (correct, total)} — dùng cho per-entity breakdown.
    Greedy matching: mỗi predicted entity chỉ match 1 lần.
    """
    unmatched = [e["entity"] for e in got]
    totals = {}
    for e in expected:
        t = e["entity"]
        totals[t] = totals.get(t, [0, 0])
        totals[t][1] += 1
        if t in unmatched:
            unmatched.remove(t)
            totals[t][0] += 1
...
def entity_accuracy(expected: list, got: list) -> tuple:
    ...
    Trả về (correct, total) — đếm entity TYPES đúng, greedy matching.
    FIX: dùng list thay set để tránh over-count khi 1 câu có 2 entity cùng type.
    """
    if not expected:
        return 0, 0   # không có entity → không tính vào denominator

    # greedy: mỗi predicted entity chỉ match 1 lần (không dùng set)
    unmatched = [e["entity"] for e in got]
    correct = 0
    for e in expected:
        t = e["entity"]
...
def main():
    ...
    by_intent_intent = {}
    by_intent_entity = {}
    by_entity = {}          # {entity_type: [correct, total]}
    intent_ok_total = 0
    entity_ok_slots = 0
    ...
    for i, tc in enumerate(test_cases):
        ...
        for etype, (c, t) in entity_per_type(expected_ents, got_entities).items():
    ...
    # per-entity type breakdown
    print(f"\n{'═'*100}")
    print(f"  {BOLD}SLOT RECALL — per entity type{RESET}  (so sánh với nlu_eval)")
    print(f"{'─'*100}")
    print(f"  {'Entity':<24} {'Correct':>8} {'Total':>7} {'Recall':>8}")
    print(f"  {'─'*24} {'─'*8} {'─'*7} {'─'*8}")
    entity_order = ["drink", "size", "quantity", "category", "ingredient",
                    "payment_method", "price_limit", "info_type"]
    for etype in entity_order:
        if etype not in by_entity:
            continue
        c, t = by_entity[etype]
        r = c / t if t > 0 else 1.0
        ec = GREEN if r >= 0.9 else (YELLOW if r >= 0.8 else RED)
        print(f"  {etype:<24} {ec}{c:>8}{RESET} {t:>7} {ec}{r*100:>7.1f}%{RESET}")
    print(f"{'═'*100}\n")
    ...
    with open(out_path, "w", encoding="utf-8") as f:
        json.dump({
            ...
            "slot_per_entity":    {k: {"correct": v[0], "total": v[1],
                                       "recall": round(v[0]/v[1], 4) if v[1] > 0 else 1.0}
